...
function generatePages(TABLE_ID, PAGINATOR_ID, PAGESIZEDDL_ID, overridePageSizeDDL) {
    tb = $('#'+ TABLE_ID);
    pag = $('#' + PAGINATOR_ID);
    ddl = $('#' + PAGESIZEDDL_ID);
    pg();
...
function sortTable(sender, compare_type) {
    var table, n, rows, switching, i, x, y, shouldSwitch, dir, switchcount = 0;
	
	n = $(sender).index();	//column index
	
	table = document.getElementById($(sender).closest('table').attr('id'));
